...
- **Get Viewers**
  ...
  - API VkLive отдаёт не больше 200 зрителей в этом списке.
  - В аргумент `viewers_count` попадает размер полученного списка (не больше 200). При большем числе зрителей на канале это значение может **не совпадать** с фактическим количеством зрителей.

- **Get Viewers Count**
  - Запрашивает у API **полное** число зрителей на канале и записывает его в аргумент `viewers_count`.
  - Используйте этот метод, если нужна именно общая цифра, а не размер обрезанного списка из **Get Viewers**.

- **Get Random Viewer**
  - Возвращает случайного зрителя из списка *активных* зрителей (тот же источник, что и у списка участников чата; до 200 ников).
  - Имя записывается в аргумент `randomUserName0`.
...
Если интеграция MiniChat не установлена/не импортирована, то экшены сервиса могут выполняться, но **события в MiniChat отображаться не будут**.
